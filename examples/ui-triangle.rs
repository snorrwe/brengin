...
impl TrianglePipeline {
    fn new(renderer: &GraphicsState) -> Self {
        let render_pipeline_layout =
            renderer
                ...
                .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: Some("UI Triangle Render Pipeline Layout"),
                    bind_group_layouts: &[],
        ...
        let shader = renderer
            .device()
            .create_shader_module(include_wgsl!("assets/triangle-shader.wgsl"));

        let render_pipeline =
            renderer
                .device()
                .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                    label: Some("UI Triangle Render Pipeline"),
...
impl<'a> RenderCommand<'a> for TrianglePipeline {
    type Parameters = Res<'a, TrianglePipeline>;
...
impl Plugin for GamePlugin {
    fn build(self, app: &mut brengin::App) {
        ...
        app.add_plugin(RenderCommandPlugin::<TrianglePipeline>::new(
...
fn setup_pipeline(mut cmd: Commands, graphics_state: Res<GraphicsState>) {
    let pl = TrianglePipeline::new(&graphics_state);
